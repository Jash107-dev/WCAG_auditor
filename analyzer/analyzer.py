...
def run_analyzer(page):
    ...
    check_broken_links(soup, page, issues_list, allowed)
    ...
    for img in all_imgs:
        ...
        if not alt_text:
            new_issue = Issue.objects.create(
                ...
                rule=rule,
                severity="critical",
                message="Image is missing alt text: " + str(img.get("src", "no src")),
    ...
    for inp in all_inputs:
        ...
        if found_label == False and not aria and not aria2:
            new_issue = Issue.objects.create(
                ...
                rule=rule,
                severity="serious",
                message="Input field has no label: " + str(inp.get("name", "unknown")),
                fix=rule.fix_suggestion
            )
            issues_list.append(new_issue)
    ...
    if html_tag:
        ...
        if not lang:
            new_issue = Issue.objects.create(
                ...
                rule=rule,
                severity="serious",
                message="The html tag does not have a lang attribute",
                fix=rule.fix_suggestion
            )
            issues_list.append(new_issue)
...
def check_page_title(soup, page, issues_list, allowed):
    ...
    if not title_tag:
        new_issue = Issue.objects.create(
            ...
            rule=rule,
            severity="serious",
            message="Page has no title tag at all",
            fix=rule.fix_suggestion
        )
        issues_list.append(new_issue)
    else:
        ...
        if title_text == "":
            new_issue = Issue.objects.create(
                ...
                rule=rule,
                severity="serious",
                message="Page title tag is empty",
                fix=rule.fix_suggestion
            )
            issues_list.append(new_issue)
    ...
    while i < len(heading_levels):
        ...
        if diff > 1:
            new_issue = Issue.objects.create(
                ...
                rule=rule,
                severity="moderate",
                message="Heading jumps from h" + str(heading_levels[i-1]) + " to h" + str(heading_levels[i]),
                fix=rule.fix_suggestion
            )
            issues_list.append(new_issue)
            break
    ...
    for lnk in all_links:
        ...
        if txt in bad_link_texts:
            new_issue = Issue.objects.create(
                ...
                rule=rule,
                severity="moderate",
                message="Link text is not descriptive: " + lnk.get_text(strip=True),
                fix=rule.fix_suggestion
            )
            issues_list.append(new_issue)
    ...
    for btn in all_buttons:
        ...
        if not btn_text and not aria_lbl and not aria_lbl2:
            new_issue = Issue.objects.create(
                ...
                rule=rule,
                severity="critical",
                message="Button has no text and no aria-label",
                fix=rule.fix_suggestion
            )
            issues_list.append(new_issue)
    ...
    for el in all_elements:
        ...
        if has_lbl == False and not el.get("aria-label"):
            new_issue = Issue.objects.create(
                ...
                rule=rule,
                severity="serious",
                message="Form element is missing a label: " + el.name,
                fix=rule.fix_suggestion
            )
            issues_list.append(new_issue)
...
def check_broken_links(soup, page, issues_list, allowed):
    ...
    for a in all_links:
        ...
        try:
            res = requests.head(href, timeout=4, allow_redirects=True)
            if res.status_code >= 400:
                new_issue = Issue.objects.create(
                    page=page,
                    rule=rule,
                    severity="serious",
                    message="Broken link found (" + str(res.status_code) + "): " + href,
                    fix="Remove or fix this broken link"
                )
                issues_list.append(new_issue)
        except:
            pass
                ...
                if rgb1 and rgb2:
                    ...
                    if ratio < 4.5:
                        new_issue = Issue.objects.create(
                            ...
                            rule=rule,
                            severity="serious",
                            message="Color contrast is too low " + str(round(ratio, 2)) + ":1 on " + el.name,
                            fix=rule.fix_suggestion
                        )
                        issues_list.append(new_issue)
